...
핸드오프 문서는 **다음 작업자가 실제로 읽는 분량**이어야 한다. 길면 안 읽고, 안 읽으면 진입점이 아니다.

| 산출물 | 분량 예산 | 기준 |
|---|---|---|
| `docs/handoff/HANDOFF.md` | ≤ 60줄 | 한 화면. 항목 하나 = 한 줄 |
| `docs/sessions/*.md` | ≤ 80줄 | 무엇을 왜 바꿨는지. 코드·로그 전문 붙여넣기 금지 |
| `docs/decisions/ADR-*.md` | ≤ 40줄 | 맥락/결정/대안/근거 각 3~5줄 |
| `docs/regressions/*.md` | ≤ 20줄 | 증상/영향/재발 방지 |

- **패딩 금지**: 도입 서사, 같은 내용을 다시 쓰는 요약 섹션, "이번 세션은 생산적이었습니다" 류 총평, 빈 섹션 자리채움. 내용이 없는 섹션은 헤딩째 뺀다.
- **중복 금지**: HANDOFF.md 는 세션 로그의 요약본이 아니라 **진입점**이다. 상세는 `docs/sessions/` 에 두고 HANDOFF 에서는 경로로 가리킨다.
- **위임 금지**: 0~6단계는 현재 세션에서 직접 수행한다. 단계별로 서브에이전트를 띄우지 않는다 — 상태 수집·회귀 가드·harness 점검은 서로의 결과를 봐야 판단이 서므로 쪼개면 품질이 떨어진다. 예외는 넓은 다중 파일 조사 1건이고, `docs/` 쓰기는 항상 메인 세션이 한다.
- **보고는 얇게**: 시작에 한 문장, 중간에는 회귀·미추적 파일처럼 사용자가 알아야 할 것을 발견했을 때만, 마지막은 결과부터 (6단계).
...
- **발견한 문제는 보고만 하지 말고 보완한다.** 단, 보완 범위는 **이번 세션이 바꾼 것 + 다음 작업자를 막을 것**까지다. 핸드오프는 harness 리팩터링 세션이 아니다 — 전면 재구조화·재작성이 필요해 보이면 실행하지 말고 HANDOFF.md `⚠️` 에 제안 한 줄로 남긴다.
...
### 6. 마무리 보고
- **첫 문장이 결과다** — "핸드오프 완료. 다음 첫 액션: {한 줄}". 수행한 절차를 다시 나열하지 않는다.
- 이어서 생성·갱신한 파일 목록과 내린 가정을 한눈에 요약한다.
