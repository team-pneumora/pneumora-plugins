...
기록 후 해당 파일의 Regression Log 항목 수를 센다. **6건 이상**이면:

1. 아카이브 파일 결정: `docs/REGRESSIONS.md` (git 루트 기준, `docs/` 없으면 생성)
2. **최신 5건만** CLAUDE.md에 남기고, 나머지 오래된 항목을 아카이브 파일 끝에 **원문 그대로, 시간순 보존**하여 이동
3. CLAUDE.md의 Regression Log 섹션 마지막에 포인터 한 줄 유지 (이미 있으면 건수만 갱신):
   ```
   - 이전 회귀 이력 → `docs/REGRESSIONS.md` (N건)
   ```
4. 아카이브 파일이 없으면 헤더와 함께 생성:
   ```markdown
   # Regression Archive
   <!-- CLAUDE.md Regression Log 에서 로테이션된 이력. 원문 보존, 시간순. -->
   ```

**이유**: instruction 파일(특히 루트)은 매 프롬프트 상시 주입된다. 누적 섹션이 무한 증식하면
상시 토큰 누수가 된다. 아카이브는 디스크에 남아 필요할 때만 읽힌다.

로테이션은 항목을 **이동**할 뿐 수정·요약·순서 변경하지 않는다 (아래 금지 사항과 충돌 아님).
...
- ❌ Regression Log 기존 항목 순서 바꾸기 (로테이션 시 아카이브로의 시간순 이동은 허용)
- ❌ 로테이션 시 항목 내용 수정·요약 (원문 그대로 이동만)
- ❌ 서브에이전트 호출 (읽기·편집 몇 번이면 끝나는 작업 — 직접 수행)
- ❌ 회귀 원인을 조사하거나 코드를 고치기 — 이 커맨드는 **기록만** 한다. 수정은 사용자가 따로 지시
