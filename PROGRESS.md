...
- **현재 만지는 파일**: (없음 — Opus 5 가이드 반영 작업 완료, 미커밋)
- **미해결 결정**: 없음 (직전 세션의 `claude-md-harness` 미점검 건은 이번에 v1.2.0 으로 해소)
- **다음 작업 첫 단계**: 커밋 → push (계정 확인 필수) → `/plugin marketplace update pneumora-plugins` + `/reload-plugins` 로 캐시 갱신. 그다음 실제 프로젝트에서 `/ceo-dev-loop:init "목표"` 로 루프 1회 완주 — 여전히 유일한 미검증 영역
- **커밋 상태**: 미커밋 24파일 (4개 플러그인 전부 bump: 0.9.0 / 1.2.0 / 0.4.0 / 0.3.0)
- **진입점**: `docs/handoff/HANDOFF.md`
- **하네스 여유**: CLAUDE.md 784/800, AGENTS.md 797/800 — 다음 규칙 추가 시 기계 강제되는 CRITICAL(#2·#3)부터 축약

## 2026-07-27 세션 — Opus 5 프롬프트 가이드 반영 (4개 플러그인 전부 bump)

[Prompting Claude Opus 5](https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-claude-opus-5) 가이드 원문과 레포의 모든 프롬프트 파일(SKILL.md · commands · agents)을 1:1 대조. **기능 회귀가 아니라 비용 회귀** 를 막는 변경 — 안전장치는 그대로 두고, 모델이 시키지 않아도 과하게 하는 3가지에만 상한을 걸었다.

### 가이드 → 레포 매핑 (적용한 것)

| 가이드 지적 | 레포에서 걸린 곳 | 조치 |
|---|---|---|
| 서브에이전트 위임이 이전 모델보다 잦음 → 조건·상한 명시 | 위임 상한이 `pneumora/commands/critical.md` 한 줄에만 존재 | 4개 플러그인 전부에 위임 규약 추가 |
| 디스크 산출물이 길어짐 → 분량 캘리브레이션 명시 | `handoff` 는 예산 전무, `ceo-dev-loop` 는 사후 압축 게이트만 | 작성 시점 예산으로 이동/신설 |
| 에이전트 작업 중 나레이션 증가 → cadence 지정 | 자율 루프·핸드오프 모두 미지정 | 보고 cadence 명시 |
| 스코프 확장 경향 → 범위 고정 | `handoff` 3단계 "발견한 문제는 보완한다" 가 무제한 | 세션 범위 + 차단 요소로 한정 |
| 긴 시스템 프롬프트는 끝에 짧은 리마인더 | `ceo.md` 383줄, 말미 규약 없음 | `## 출력 규약` 추가 |

### 플러그인별

- **ceo-dev-loop v0.8.0 → v0.9.0**: `init.md` CLAUDE.md 템플릿에 `### 실행 밀도 규약` 신설 — 위임은 `@ceo` 하나(병렬 에이전트가 STATUS/DECISIONS 동시 쓰기 → 핸드오프 붕괴 + hook mtime 오염), Dev 의 자체 재검증 라운드 금지(검증은 CEO 교차검증 구조라 writer-verifier 는 유지), 3파일 분량 예산을 작성 기준으로 앞당김, 보고 cadence. `ceo.md` 말미 `## 출력 규약` 추가. `start.md` · SKILL.md 동기
- **handoff v0.2.1 → v0.3.0**: `## 실행 밀도` 신설 — 산출물별 분량 예산표(HANDOFF ≤ 60줄 / sessions ≤ 80 / ADR ≤ 40 / regressions ≤ 20), 패딩·중복 금지, 단계별 팬아웃 금지(0~6단계는 서로의 결과를 봐야 판단이 서므로 쪼개면 품질 저하), 마무리 보고는 결과부터. 3단계 harness 보완 스코프를 "이번 세션이 바꾼 것 + 다음 작업자를 막을 것" 으로 한정
- **pneumora v0.3.0 → v0.4.0**: `check-deploy` · `log-regression` 금지 사항에 서브에이전트 호출 + **보고까지가 범위(발견한 문제를 대신 고치지 않음)** 추가. `codebase-explorer` 에 `effort: medium` — ultracode/xhigh 세션에서 탐색 에이전트가 상속받던 과잉 effort 차단
- **claude-md-harness v1.1.0 → v1.2.0**: 압축을 사후 보정이 아닌 **작성 기준**으로 명시, 재감사 스코프를 lint 보고 항목으로 한정, **파일당 서브에이전트 팬아웃 금지** — 계층 간 DRY·배치 휴리스틱은 전 계층을 한 컨텍스트에서 봐야 성립하므로 파일별로 쪼개면 중복을 구조적으로 못 찾는다

### 하네스

루트 `CLAUDE.md`/`AGENTS.md` Global Rules 에 **프롬프트 작성 4원칙** 추가(① 자기 재검증 지시 금지 ② 위임 조건·상한 ③ 산출물 분량 예산 ④ 보고 cadence). 추가로 두 파일이 예산 초과(902/908) → 압축 패스로 784/797 복귀. 직전 세션 메모("7번째 규칙 추가 시 축약")대로 처리.

### 검증

`validate-plugins.sh` ✅ (4개 버전 동기, 두 marketplace 파싱, 좀비 0) · `harness-lint.sh` ✅ CLAUDE.md·AGENTS.md 경고 0건.
