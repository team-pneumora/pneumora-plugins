---
name: github-triage
description: Triage GitHub issues through a label-based state machine. Use when user wants to create an issue, triage issues, review incoming bugs or feature requests, prepare issues for an AFK agent, or manage issue workflow. 한국어 트리거 — "이슈 트리아지", "needs-triage 처리", "ready-for-agent 라벨", "이슈 정리 도와줘".
---

> Adapted from [mattpocock/skills/github-triage](https://github.com/mattpocock/skills/tree/main/github-triage) (MIT). Body kept in English; Korean triggers added.
...
2. If no GitHub remote exists, refuse and explain: "This skill operates on GitHub issues. Set up a GitHub remote first."
...
- Want to flesh it out → start a `domain-model` session (from the `domain-language` plugin)
...
If the issue needs to be fleshed out before it's ready for an agent, interview the maintainer to build a complete specification. Use the `domain-model` skill from the `domain-language` plugin.
