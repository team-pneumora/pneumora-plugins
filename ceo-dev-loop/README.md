...
> **v0.9.0** — 실행 밀도: 위임은 `@ceo` 하나로 고정, Dev 의 자체 재검증 라운드 금지, 핸드오프 3파일 작성 시점 분량 예산, 사용자 보고 cadence, CEO 출력 규약
...
## 기존 프로젝트 업그레이드 (v0.8.0 → v0.9.0)

v0.9.0 은 규칙 추가뿐이고 파일 포맷·센티널·hook 변경이 없습니다. 아래 0단계(플러그인 캐시 갱신)를 먼저 하면 `agents/ceo.md` · `commands/*` 는 자동 반영되고, 프로젝트 산출물은 한 곳만 손보면 됩니다.

**프로젝트 `CLAUDE.md`** 의 `## CEO-Dev 자동 루프 규칙` 안에 `### 실행 밀도 규약` 섹션을 추가하세요 (`commands/init.md` 의 해당 블록을 그대로 복사). 없어도 루프는 돌지만, 이게 없으면 Dev 가 검증용 서브에이전트를 띄우고 STATUS/DECISIONS 를 장문으로 쓰기 시작해 **CEO 호출마다 3파일을 전부 읽는 비용이 계속 올라갑니다.**

가장 빠른 방법 — 대상 프로젝트에서 한 줄 지시:

> **"ceo-dev-loop v0.9.0 의 '실행 밀도 규약' 섹션을 프로젝트 CLAUDE.md 의 CEO-Dev 자동 루프 규칙 안에 추가해줘 — 위임은 @ceo 하나, 자체 재검증 라운드 금지, STATUS/DECISIONS 분량 예산, 사용자 보고 최소화."**
...
### v0.9.0 (실행 밀도 — Opus 5 프롬프트 가이드 반영)
Anthropic 의 [Prompting Claude Opus 5](https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-claude-opus-5) 가이드와 이 플러그인의 프롬프트를 대조해 나온 수정. 기능 회귀가 아니라 **비용 회귀** 를 막는 변경이다 — 안전장치는 그대로 두고, 모델이 스스로 과하게 하는 세 가지(위임·자기검증·장문 서술)에만 상한을 걸었다.

- **위임 상한 (`init.md` CLAUDE.md 템플릿 · `start.md` · SKILL.md)**: Opus 5 는 이전 모델보다 서브에이전트를 훨씬 잘 띄운다. 이 루프에서 그건 그냥 비용이 아니라 **정합성 문제** — 병렬 에이전트가 `docs/STATUS.md` · `docs/DECISIONS.md` 를 동시에 쓰면 핸드오프 상태가 깨지고, hook 이 진척 신호로 읽는 mtime 도 오염된다. 위임은 `@ceo` 하나로 고정하고, 예외(넓은 다중 파일 탐색 1건)에서도 `docs/` 쓰기는 메인 세션이 독점한다
- **자체 재검증 라운드 금지**: 가이드는 "Opus 5 는 시키지 않아도 자기 작업을 검증하므로, 명시적 검증 지시는 **과잉 검증**을 유발한다" 고 못박는다. 이 플러그인의 검증은 원래 **CEO 가 Dev 를 교차 검증**하는 writer-verifier 구조라 그대로 두되 (가이드도 이 패턴은 권장), Dev 쪽에 "검증 명령 1회 → 출력 그대로 STATUS.md → `@ceo`" 를 명시해 **같은 축의 검증이 Dev·CEO 양쪽에서 중복되는 것**을 막았다
- **분량 예산을 작성 시점으로 이동**: v0.8.0 의 압축 게이트(DECISIONS 150줄 / STATUS 120줄)는 **비대해진 뒤** 완료 경계에서 되돌리는 사후 보정이었다. 가이드가 지적한 "디스크에 쓰는 산출물이 이전 모델보다 길어진다" 를 감안하면 그사이 모든 CEO 호출이 비싼 파일을 읽는다 — 같은 숫자를 **작성 기준**으로 앞당기고 (항목 1줄, 증거는 판정에 필요한 출력만, 이전 턴 재요약 금지) 게이트는 backstop 으로 남겼다
- **보고 cadence 명시**: 루프 중 사용자는 읽지 않고 나중에 몰아본다. 턴 시작 한 문장 → 작업 → 결과 한 줄, 전체 요약은 `[DONE]` 보고에서만
- **CEO 출력 규약 (`agents/ceo.md` 말미)**: 383줄짜리 긴 프롬프트라 가이드 권고대로 **끝에 짧은 리마인더**를 붙였다 — 응답 형식 밖으로 새지 않기, 평상시 20줄 이내, 판정 먼저·근거는 뒤, 결론이 바뀔 때만 자기 정정 언급, GOAL 밖 아이디어는 지시가 아니라 기록으로
